...
try {
  ...
  await test('INSERT d\'un seul produit', async () => {
    // TODO 1 : Inserez un produit (name='Clavier', price=49.99, category='Informatique')
    // Indice : INSERT INTO products (name, price, category) VALUES (...)
  ...
  await test('INSERT multiple avec RETURNING', async () => {
    // TODO 2 : Inserez plusieurs produits en une seule requete et recuperez-les avec RETURNING
    // Produits a inserer :
    ...
    //
    // Indice : INSERT INTO products (...) VALUES (...), (...), (...) RETURNING *
  ...
  await test('SELECT avec WHERE', async () => {
    // TODO 3 : Recuperez tous les produits de la categorie 'Informatique'
    // Indice : SELECT * FROM products WHERE category = ...
  ...
  await test('Agregation AVG par categorie', async () => {
    // TODO 4 : Calculez le prix moyen par categorie avec AVG() et GROUP BY
    // Indice : SELECT category, AVG(price) AS avg_price FROM products GROUP BY category ORDER BY category

    let result: Awaited<ReturnType<typeof query>>; // <-- remplacez par votre requete

    assertEqual(result!.rows.length, 2, 'Il devrait y avoir 2 categories');
    // Informatique : (49.99 + 29.99 + 299.99) / 3 = 126.66
    const infoRow = result!.rows.find((r: { category: string }) => r.category === 'Informatique');
  ...
  await test('UPDATE avec RETURNING', async () => {
    // TODO 5 : Mettez a jour le prix du 'Clavier' a 59.99 et recuperez la ligne modifiee
    // Indice : UPDATE products SET price = ... WHERE name = ... RETURNING *
  ...
  await test('DELETE avec RETURNING', async () => {
    // TODO 6 : Supprimez le produit 'Bureau' et recuperez la ligne supprimee
    // Indice : DELETE FROM products WHERE name = ... RETURNING *
  ...
  await test('Requetes parametrees ($1, $2)', async () => {
    // TODO 7 : Faites un SELECT avec 2 parametres : category ET un prix minimum
    // Requete : SELECT * FROM products WHERE category = $1 AND price > $2
    // Parametres : ['Informatique', 40]

    let result: Awaited<ReturnType<typeof query>>; // <-- remplacez par votre requete

    assert(result!.rows.length > 0, 'La requete parametree devrait retourner des resultats');
    result!.rows.forEach((row: { category: string; price: string }) => {
  ...
  await test('Protection contre l\'injection SQL', async () => {
    // TODO 8 : Montrez que les requetes parametrees protegent contre l'injection SQL
    // Essayez de passer une valeur malicieuse comme parametre :
    //   maliciousInput = "'; DROP TABLE products; --"
    // Faites un SELECT avec $1 et cette valeur malicieuse
    // La requete doit retourner 0 lignes sans erreur (la table n'est PAS supprimee)

    const maliciousInput = "'; DROP TABLE products; --";

    // Indice : await query(client!, 'SELECT * FROM products WHERE name = $1', [maliciousInput]);
