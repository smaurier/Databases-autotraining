...
// ---------------------------------------------------------------------------
async function resetBalances(c) {
...
// ---------------------------------------------------------------------------
async function safeTransfer(c, fromId, toId, amount) {
  ...
  try {
    ...
    await test('NOWAIT evite le deadlock en echouant rapidement', async () => {
      ...
      try {
        ...
        try {
          ...
          noWaitError = true;
          assertEqual(err.code, '55P03', 'Doit recevoir 55P03 (lock_not_available)');
        }
    ...
    await test('Traitement par lots avec ORDER BY pour eviter les deadlocks', async () => {
      ...
      try {
        ...
        const batchUpdate = async (c, amount) => {
          await query(c, 'BEGIN');
          await query(c,
